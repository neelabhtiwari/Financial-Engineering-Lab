...
bd1=readtable('wbsedata1.csv');
bd=bd1{:,2:end};
nd1=readtable('wnsedata1.csv');
nd=nd1{:,2:end};
bd2=bd;
nd2=nd;
...
munse=sum(nd(53:end,:))/3;
sigmanse=sqrt(var(nd(53:end,:)));
mubse=sum(bd(53:end,:))/3;
sigmabse=sqrt(var(bd(53:end,:)));
...
for i = 1:size(nse_titles,2)
    ...
    S0=nd2(53,i);
    for j=1:52
        inpower=(munse(i)-(sigmanse(i)*sigmanse(i))/2)*j*(1/52)+sigmanse(i)*normrnd(0,j*(1/52));
        predict(j)=S0*exp(inpower);
    end    
    
    figure(1),hold on
    plot(predict,'Linewidth',linewidth);
    orgplot=flipud(nd2(1:52,i));
...
for i = 1:size(bse_titles,2)
    ...
    S0=bd2(53,i);
    for j=1:53
        inpower=(mubse(i)-(sigmabse(i)*sigmabse(i))/2)*j*(1/52)+sigmabse(i)*normrnd(0,j*(1/52));
        predict(j)=S0*exp(inpower);
    end 
    
    figure(1),hold on
    plot(predict,'Linewidth',linewidth);
    orgplot=flipud(bd2(1:52,i));
    plot(orgplot,'Linewidth',linewidth);
    xlabel('Weekly basis');
    ylabel('Prices');
    hold off;
