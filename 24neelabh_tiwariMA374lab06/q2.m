...
nd= (nd(1:end-1,:)-nd(2:end,:))./nd(2:end,:);
nw = (nw(1:end-1,:)-nw(2:end,:))./nw(2:end,:);
nm = (nm(1:end-1,:)-nm(2:end,:))./nm(2:end,:);

bd= (bd(1:end-1,:)-bd(2:end,:))./bd(2:end,:);
bw = (bw(1:end-1,:)-bw(2:end,:))./bw(2:end,:);
bm = (bm(1:end-1,:)-bm(2:end,:))./bm(2:end,:);
...
for i = 1:size(nse_titles,2)
    ...
    xlabel('Weekly basis');
    ylabel('Normalized returns');
    title(sprintf('NSE -- Normalized Returns -- %s',nse_titles(i)));
    subplot(1,3,3)
    temp_data = nm(:,i);
    temp=flipud(temp_data);
    temp_mean = mean(temp_data);
    ...
    saveas(gcf,sprintf('NSE_Return_%d.png',i));
end
...
for i = 1:size(bse_titles,2)
    ...
    xlabel('Weekly basis');
    ylabel('Normalized returns');
    title(sprintf('BSE -- Normalized returns -- %s',bse_titles(i)));
    subplot(1,3,3)
    temp_data = bm(:,i);
    temp=flipud(temp_data);
    temp_mean = mean(temp_data);
    ...
    ylabel('Normalized returns');
    set(gcf, 'Units', 'Inches', 'Position', [0, 0, 15.25, 8], 'PaperUnits', 'Inches', 'PaperSize', [7.25, 9.125])
    saveas(gcf,sprintf('BSE_Return_%d.png',i));
end
